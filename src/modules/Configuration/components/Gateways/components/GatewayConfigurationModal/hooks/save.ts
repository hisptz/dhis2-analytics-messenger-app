...
export function useSaveGateway() {
	...
	const [create, { loading: creating }] = useDataMutation(
		generateGatewayCreateMutation(id),
		{
			onComplete: () => {
				show({
					message: i18n.t("Gateway config saved successfully"),
					type: { success: true },
				});
			},
			onError: (error) => {
				show({
					message: `${i18n.t(
						"Could not save gateway information",
					)}: ${error.message}`,
					type: { critical: true },
				});
	...
	const [update, { loading: updating }] = useDataMutation(
		updateGatewayMutation,
		{
			onComplete: () => {
				show({
					message: i18n.t("Gateway config updated successfully"),
					type: { success: true },
				});
			},
			onError: (error) => {
				show({
					message: `${i18n.t(
						"Could not update gateway information",
					)}: ${error.message}`,
					type: { critical: true },
				});
				setTimeout(hide, 5000);

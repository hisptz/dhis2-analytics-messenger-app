import { Button, IconRedo24 } from "@dhis2/ui";
...
export function GatewayCleanupButton({
	gateway,
	channel,
	...
	const handleDelete = async () => {
		return await new Promise((resolve, reject) => {
			confirm({
				...
				onConfirm: async () => {
					await Parse.Cloud.run("cleanupWhatsappClient", {
	...
	const { mutate, isLoading } = useMutation({
		...
		onSuccess: () => {
			show({
				message: i18n.t("Gateway cleaned up successfully"),
				type: { success: true },
			});
			refetch();
		},

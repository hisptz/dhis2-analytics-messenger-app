...
const Wrapper = ({
	...
	<div className="row gap-8 align-center">
		<Tooltip
			content={
				customMessage ?? (
					<div style={{ gap: 2 }} className="column ">
						<span>
							<b>{i18n.t("Status")}:</b>{" "}
...
export function StatusIndicator({ value }: StatusIndicatorProps) {
	...
	const { isError, isLoading, data, refetch, isRefetching, error } = useQuery<
		StatusData,
		AxiosError
	>([value], getStatus);
	...
	if (isError) {
		return (
			<Wrapper
				customMessage={<span>{error?.message}</span>}

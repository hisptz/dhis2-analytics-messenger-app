...
export function useVisualizationGroups(onEdit: () => void): {
	...
	const { data, loading, error, refetch } = useDataQuery<{
		visualizationGroups: {
			pager: any;
			entries: (VisualizationGroup & { key: string })[];
		};
	}>(query);
	...
	const [deleteGroup] = useDataMutation(deleteMutation, {
		onComplete: () => {
			show({
				message: i18n.t("Visualization group deleted successfully"),
				type: { success: true },
			});
			refetch();
		},
		onError: (err) => {
			show({
				message: `${i18n.t("Error deleting group")}: ${err.message}`,
				type: { critical: true },
			});
		},
	});

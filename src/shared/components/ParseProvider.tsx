...
import { GatewayConfigurationModal } from "../../modules/Configuration/components/Gateway/components/GatewayConfigurationModal";
...
export function ParseProvider({ children }: { children: React.ReactNode }) {
	...
	const loginToParse = useParseLogin();
	const [config] = useSetting("gatewayConfig", { global: true });
	...
	useEffect(() => {
		if (!config || !config.token) {
			...
			try {
				loginToParse().catch((error) => {
					show({
						message:
							i18n.t(
								"Could not connect to DHIS2 analytics messenger service",
							) + ` ${error.message}`,
						type: { critical: true },
						actions: [
							{
	...
	return (
		<>
			{!hide && (
				<GatewayConfigurationModal onClose={onHide} hide={hide} />
			)}
			{children}
		</>
	);
